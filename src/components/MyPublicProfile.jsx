...
export default function MyPublicProfile({ session, userProfile: initialUserProfile }) {
  ...
  const fetchMyProfileData = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Erreur", description: "Impossible de charger votre profil ou vos recettes.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }, [session, toast]);
  ...
  if (!profileData && !session?.user?.id) {
    return (
      <div className="text-center py-12 section-card">
        <h1 className="text-2xl font-bold text-pastel-primary mb-4">Non connecté</h1>
        <p className="text-pastel-muted-foreground">Veuillez vous connecter pour voir votre aperçu de profil.</p>
      </div>
    );
  }
  ...
  if (!profileData && session?.user?.id) {
     return (
      <div className="text-center py-12 section-card">
        <h1 className="text-2xl font-bold text-pastel-primary mb-4">Erreur de chargement</h1>
        <p className="text-pastel-muted-foreground">Impossible de charger votre profil. Veuillez réessayer.</p>
      </div>
    );
  }
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-8">
            ...
            <div className="flex-grow text-center sm:text-left">
              <div className="flex items-center gap-2 justify-center sm:justify-start mb-1">
                <h1 className="text-3xl sm:text-4xl font-bold text-pastel-primary">{profileData.username}</h1>
                <ShieldCheck className="w-7 h-7 text-pastel-tertiary" title="Ceci est votre aperçu de profil public"/>
              </div>
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-x-4 gap-y-2 text-sm text-pastel-muted-foreground">
               ...
               <p className="text-xs text-pastel-muted-foreground mt-3 italic">C'est ainsi que les autres utilisateurs voient votre profil (recettes publiques et pour amis).</p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {recipes.length > 0 ? (
            ...
            <div className="text-center py-10 section-card">
              <p className="text-xl text-pastel-muted-foreground">Vous n'avez pas encore partagé de recettes (ou elles sont toutes privées).</p>
            </div>
          )}
        </div>
      </div>
