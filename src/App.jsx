...
import { Routes, Route, useLocation, Navigate, useNavigate as useRouterNavigate } from 'react-router-dom';
...
function App() {
  ...
  useEffect(() => {
    ...
    } else if (location.pathname === '/app' || location.pathname === '/app/' || location.pathname === '/') {
      setActiveTab('recipes');
      if(location.pathname !== '/app/recipes') routerNavigate('/app/recipes', { replace: true });
    }
  ...
  const fetchUserProfile = useCallback(async (currentSession) => {
    const defaultProfileBase = { 
      ...
      avatar_url: null,
      bio: '',
      preferences: { servingsPerMeal: 4, maxCalories: 2200, meals: [], tagPreferences: [], commonMenuSettings: { enabled: false, linkedUsers: [] } } 
    };
    ...
    try {
      ...
      if (profileError && profileError.code !== 'PGRST116') { 
        console.warn("Warning fetching public_users profile:", profileError.message, profileError.details, profileError.hint);
      }
      ...
      let finalProfileData = {
        ...
        username: profile?.username || userMetadata.username || currentSession.user.email?.split('@')[0] || 'Utilisateur',
        user_tag: profile?.user_tag || userMetadata.user_tag || ('user_' + currentSession.user.id.substring(0,8)), 
        avatar_url: profile?.avatar_url || userMetadata.avatar_url || null,
        bio: profile?.bio || userMetadata.bio || '',
        subscription_tier: userMetadata.subscription_tier || appMetadata.subscription_tier || 'standard',
      };
      
      const defaultPreferences = { servingsPerMeal: 4, maxCalories: 2200, meals: [], tagPreferences: [], commonMenuSettings: { enabled: false, linkedUsers: [] } };
      ...
      finalProfileData.preferences.commonMenuSettings = {
        ...
        linkedUsers: Array.isArray(userMetadata.preferences?.commonMenuSettings?.linkedUsers) ? userMetadata.preferences.commonMenuSettings.linkedUsers : []
      };
      ...
      toast({ title: "Erreur Profil", description: "Impossible de traiter les données du profil.", variant: "destructive" });
      setUserProfile({
        id: currentSession.user.id,
        ...defaultProfileBase,
      ...
      return {id: currentSession.user.id, ...defaultProfileBase, user_tag: ('user_' + currentSession.user.id.substring(0,8))};
    }
  }, [toast]);
  ...
  const handleEditRecipeSubmit = async (recipeData) => {
    ...
    if (success) { setShowRecipeForm(false); setEditingRecipe(null); }
    return success;
  };
  ...
  const openRecipeFormForAdd = () => { setEditingRecipe(null); setShowRecipeForm(true); };
  const closeRecipeForm = () => { setShowRecipeForm(false); setEditingRecipe(null); };
  ...
  const handleProfileUpdated = useCallback(async () => {
     ...
     } catch (error) { console.error("Error on handleProfileUpdated (fetchUserProfile):", error); } 
  ...
  const isMenuDataEmpty = (menu) => {
    ...
    return !menu.some(dayMeals => Array.isArray(dayMeals) && dayMeals.some(mealRecipes => Array.isArray(mealRecipes) && mealRecipes.length > 0));
  };
  ...
  return (
    <div className={`min-h-screen text-pastel-text bg-pastel-background transition-colors duration-300 ${darkMode ? 'dark' : ''} flex flex-col`}>
      <MainAppLayout 
        session={session}
          ...
          <Route path="/app/recipes" element={
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 section-card">
                <h2 className="text-2xl sm:text-3xl font-bold text-pastel-primary">{recipePageTitle}</h2>
                <button onClick={openRecipeFormForAdd} className="bg-pastel-secondary text-pastel-secondary-text hover:bg-pastel-secondary-hover px-4 py-2 rounded-md shadow-pastel-button hover:shadow-pastel-button-hover flex items-center">
                  <PlusCircle className="w-5 h-5 mr-2" />Ajouter une Recette
                </button>
              </div>
              {recipesLoading ? 
                <LoadingScreen message="Chargement des recettes..." />
                : <RecipeList recipes={recipes} onEdit={(recipe) => { setEditingRecipe(recipe); setShowRecipeForm(true); }} onDelete={deleteRecipeHook} onSelectRecipe={setSelectedRecipeForDetail} />
              }
              {showRecipeForm && <RecipeForm recipe={editingRecipe} onSubmit={editingRecipe ? handleEditRecipeSubmit : handleAddRecipeSubmit} onClose={closeRecipeForm} session={session} userProfile={userProfile} />}
            </div>
          } />
          <Route path="/app/menu" element={
             (recipesLoading || weeklyMenuLoading) && isMenuDataEmpty(weeklyMenu) ? 
              <LoadingScreen message="Chargement du menu..." />
              : <MenuPlanner recipes={recipes} weeklyMenu={weeklyMenu} setWeeklyMenu={saveUserWeeklyMenuHook} userProfile={userProfile} />
          } />
          <Route path="/app/shopping" element={
            (recipesLoading || weeklyMenuLoading) && isMenuDataEmpty(weeklyMenu) ?
           ...
           <Route path="/app/community" element={
            session ? <CommunityPage session={session} userProfile={userProfile} /> : <Navigate to="/app/recipes" replace />
          } />
          <Route path="/app/profile/:userId" element={
            <UserProfilePage session={session} currentUserProfile={userProfile} />
          } />
          <Route path="/app/account" element={
            session && (recipesLoading || weeklyMenuLoading) && isMenuDataEmpty(weeklyMenu) ? 
              <LoadingScreen message="Chargement du compte..." />
              : session ? <AccountPage session={session} userProfile={userProfile} onProfileUpdate={handleProfileUpdated} />
      ...
      {selectedRecipeForDetail && <RecipeDetailModal recipe={selectedRecipeForDetail} onClose={() => setSelectedRecipeForDetail(null)} userProfile={userProfile} />}
