  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] modal-content-custom">
        <DialogHeader>
          <DialogTitle className="text-pastel-primary">Remplacer la recette</DialogTitle>
          <DialogDescription>
        ...
        <div className="py-4 space-y-4">
          ...
          <ScrollArea className="h-[300px] border border-pastel-border rounded-md">
            {filteredRecipes.length > 0 ? (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-pastel-muted-foreground">
                      {recipe.calories || 0} kcal, {recipe.servings || 1} portions de base
                      {recipe.author && recipe.author !== (userProfile?.username || 'Moi') && ` (par ${recipe.author})`}
                    </span>
                  </div>
                </Button>
                ) : null
              ...
              <p className="p-4 text-center text-pastel-muted-foreground">Aucune recette correspondante trouvée.</p>
            )}
          </ScrollArea>
        </div>
        <DialogFooter>
