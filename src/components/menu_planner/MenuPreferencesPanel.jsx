...
import { Plus, ChevronDown, ChevronUp, Trash2, Users, Link, Unlink, Info } from "lucide-react";
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <Input id="servingsPerMeal" type="number" value={preferences.servingsPerMeal || 4}
            onChange={handleServingsPerMealChange}
            min="1" step="1" className="max-w-xs"
          />
        </div>
        <div className="space-y-2">
          <Label htmlFor="maxCalories" className="block text-base font-medium mb-1.5">Calories max. par jour</Label>
          <Input id="maxCalories" type="number" value={preferences.maxCalories || 2200}
            onChange={(e) => setPreferences({ ...preferences, maxCalories: parseInt(e.target.value) || 0 })}
            min="500" step="50" className="max-w-xs"
          />
        </div>
      </div>
      ...
      <div className="space-y-4 pt-4 border-t border-pastel-border/70">
        <div className="flex items-center space-x-2">
          ...
          />
          <Label htmlFor="common-menu-toggle" className="text-base font-medium">Activer le menu commun</Label>
          <Dialog>
            ...
            <DialogContent className="modal-content-custom">
              <DialogHeader>
                ...
                <DialogDescription>
                  Le menu commun vous permet de planifier des repas en utilisant les recettes d'autres utilisateurs que vous avez liés. 
                  Activez cette option pour lier des utilisateurs et ajuster les ratios de contribution pour la génération de menu.
                </DialogDescription>
              </DialogHeader>
            </DialogContent>
          </Dialog>
          ...
          >
            ...
            {(preferences.commonMenuSettings?.linkedUsers || []).map((user, index) => (
              <div key={user.id} className="flex items-center justify-between gap-2 text-sm">
                <span className="truncate flex-1">{user.name} {user.isOwner && "(Vous)"}</span>
                <div className="flex items-center gap-2">
                  <Input 
                    ...
                    min="0" max="100"
                    disabled={user.isOwner && (preferences.commonMenuSettings?.linkedUsers || []).length === 1}
                  />
                  <span className="text-xs">%</span>
                  {!user.isOwner && (
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:bg-red-500/10" onClick={() => handleRemoveLinkedUser(user.id)}>
            ...
            <div className="flex gap-2 pt-2 border-t border-pastel-border/50">
              ...
              />
              <Button onClick={handleAddLinkedUser} disabled={isLinkingUser || !newLinkedUserEmail.trim()} size="sm" className="h-9">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="flex flex-col">
                  <Button type="button" variant="ghost" size="icon" onClick={() => moveMeal(index, "up")} disabled={index === 0} className="h-7 w-7"> <ChevronUp className="w-4 h-4" /> </Button>
                  <Button type="button" variant="ghost" size="icon" onClick={() => moveMeal(index, "down")} disabled={index === (preferences.meals || []).length - 1} className="h-7 w-7"> <ChevronDown className="w-4 h-4" /> </Button>
                </div>
                <Label className="font-medium text-pastel-text/90">Repas {meal.mealNumber}</Label>
              </div>
              <div className="flex items-center gap-2">
                <Button type="button" variant={meal.enabled ? "secondary" : "outline"} size="sm"
                  onClick={() => {
                    const newMeals = [...(preferences.meals || [])];
                ...
                <Button type="button" variant="ghost" size="icon" onClick={() => removeMeal(index)} className="text-red-500 hover:bg-red-500/10 hover:text-red-600 h-8 w-8"> <Trash2 className="w-4 h-4" /> </Button>
              </div>
            </div>
            
            <div className="flex flex-wrap gap-2 pt-2 border-t border-pastel-border/70">
              {Object.entries(MEAL_TYPE_OPTIONS_DATA).map(([typeKey, label]) => {
      ...
      <div className="space-y-4 pt-4 border-t border-pastel-border/70">
        <div className="flex justify-between items-center">
          ...
          <Button type="button" variant="outline" size="sm" onClick={addTagPreference} disabled={!availableTags?.length} className="shadow-pastel-button hover:shadow-pastel-button-hover">
        ...
        {(preferences.tagPreferences || []).map((pref, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <Input type="number" className="w-24" value={pref.percentage}
                onChange={(e) => updateTagPreference( index, "percentage", Math.min(100, Math.max(0, parseInt(e.target.value) || 0)))}
                min="0" max="100"
              />
              <span className="text-sm text-pastel-text/80">%</span>
            </div>
            <Button type="button" variant="ghost" size="icon" onClick={() => removeTagPreference(index)} className="text-red-500 hover:bg-red-500/10 hover:text-red-600 h-8 w-8"> <Trash2 className="w-4 h-4" /> </Button>
          </motion.div>
        ))}
      </div>
    </motion.div>
