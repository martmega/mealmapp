...
export function useWeeklyMenu(session) {
  ...
  const safeSetWeeklyMenu = useCallback((data) => {
    if (Array.isArray(data) && data.length === 7 && data.every(day => Array.isArray(day))) {
      setWeeklyMenu(data);
    } else if (data && typeof data === 'object' && data.menu_data && Array.isArray(data.menu_data) && data.menu_data.length === 7) {
  ...
  useEffect(() => {
    ...
    const fetchUserWeeklyMenu = async () => {
      ...
      try {
        ...
        toast({ title: 'Erreur', description: "Impossible de charger le menu hebdomadaire: " + err.message, variant: 'destructive' });
        safeSetWeeklyMenu(null); 
      } finally {
        setLoading(false);
      }
  ...
  const saveWeeklyMenuToSupabase = async (newMenu) => {
    const validatedMenu = Array.isArray(newMenu) && newMenu.length === 7 
      ? newMenu.map(day => Array.isArray(day) ? day.map(meal => Array.isArray(meal) ? meal.map(recipe => ({...recipe, id: recipe.id?.split('_')[0]})) : []) : []) 
