...
function RecipeForm({ recipe = null, onSubmit, onClose, session, userProfile }) {
  ...
  useEffect(() => {
    if (recipe) {
      setFormData({
        ...
        ingredients: recipe.ingredients?.map(ing => ({ ...ing, suggestedUnits: [] })) || [{ name: "", quantity: "", unit: "", suggestedUnits: [] }],
  ...
  const updateSuggestedTags = useCallback(async () => {
    ...
    if (formData.ingredients.some(i => i.name) || formData.meal_types.length > 0 || formData.name) {
      baseSuggestions = await generateTagSuggestions({...formData, mealTypes: formData.meal_types}, localExistingTags);
    }
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "servings") {
      ...
      setFormData(prev => ({ ...prev, [name]: numValue > 0 ? numValue : "" }));
    } else {
  ...
  const addIngredient = () => {
    setFormData(prev => ({
      ...prev,
      ingredients: [...prev.ingredients, { name: "", quantity: "", unit: "", suggestedUnits: [] }]
    }));
  };
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) { 
        toast({ title: "Fichier trop volumineux", description: "L'image ne doit pas dépasser 5MB.", variant: "destructive" });
        return;
      }
      if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
        toast({ title: "Format de fichier non supporté", description: "Veuillez choisir une image JPEG, PNG, ou WEBP.", variant: "destructive" });
        return;
      }
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      toast({ title: "Image téléversée", description: "Votre image a été ajoutée à la recette." });
  ...
  const handleSubmit = async (e) => {
    ...
    const { name, ingredients, instructions, meal_types, servings, visibility } = formData;

    if (!name.trim()) {
      toast({ title: "Erreur", description: "Le nom de la recette est requis.", variant: "destructive" });
      setIsSubmitting(false);
      return;
    }
    if (!ingredients.some(ing => ing.name.trim())) {
      toast({ title: "Erreur", description: "Au moins un ingrédient est requis.", variant: "destructive" });
      setIsSubmitting(false);
      return;
    }
    if (!instructions || instructions.length === 0 || instructions.every(line => line.trim() === "")) {
      toast({ title: "Erreur", description: "Les instructions sont requises.", variant: "destructive" });
      setIsSubmitting(false);
      return;
    }
     if (meal_types.length === 0) {
      toast({ title: "Erreur", description: "Veuillez sélectionner au moins un type de repas.", variant: "destructive" });
      setIsSubmitting(false);
      return;
    }
    if (isNaN(parseInt(servings, 10)) || parseInt(servings, 10) <= 0) {
        toast({ title: "Erreur", description: "Le nombre de portions doit être un nombre valide supérieur à 0.", variant: "destructive" });
        setIsSubmitting(false);
        return;
    }
    if (!visibility) {
      toast({ title: "Erreur", description: "Veuillez définir la visibilité de la recette.", variant: "destructive" });
      setIsSubmitting(false);
      return;
    }
    ...
    if (selectedFile) {
      ...
      if (uploadedUrl) {
        ...
        toast({ title: "Attention", description: "L'image n'a pas pu être téléversée. La recette sera sauvegardée sans image.", variant: "default" });
      }
    }
    ...
    const recipeDataToSubmit = {
      ...
      instructions: Array.isArray(formData.instructions) ? formData.instructions.filter(line => line.trim() !== '') : [],
     ...
     toast({
        ...
        description: (
          <div className="flex flex-col gap-2">
            ...
            <Button size="sm" variant="accent" onClick={() => { onClose(); navigate('/app/account'); }} className="mt-2">
              Passer à Premium
            </Button>
          </div>
        ),
  ...
  const generateWithAI = async (type) => {
    ...
    if (!formData.name || !formData.ingredients.some(i => i.name)) {
      toast({ title: "Information manquante", description: "Veuillez remplir au moins le nom et un ingrédient.", variant: "destructive"});
      return;
    }
    ...
    try {
      const ingredientsList = formData.ingredients.filter(i => i.name).map(i => `${i.quantity || ''} ${i.unit || ''} ${i.name}`.trim()).join(', ');
      const functionName = type === 'description' ? 'generate-recipe' : 'generate-image';
      const promptBase = type === 'description' 
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `La génération ${type === 'description' ? 'de la description' : "de l'image"} a échoué.`);
      }
      ...
      if (type === 'description') {
        ...
        toast({ title: "Description générée", description: "La description a été mise à jour." });
      } else if (type === 'image') {
        setFormData(prev => ({ ...prev, image_url: data.url })); 
        setPreviewImage(data.url); 
        setSelectedFile(null); 
        toast({ title: "Image générée", description: "L'image a été mise à jour." });
      }
    } catch (error) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 bg-pastel-text/40 backdrop-blur-md flex items-center justify-center z-50 p-4"
        onClick={onClose}
      >
        <motion.div
          ...
          className="bg-pastel-card text-pastel-text rounded-xl p-6 sm:p-8 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-pastel-medium"
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-6">
                ...
                />
                 <div className="space-y-2">
                  <Label htmlFor="visibility" className="text-pastel-text/90">Visibilité</Label>
                  <Select onValueChange={handleVisibilityChange} defaultValue={formData.visibility}>
                    <SelectTrigger id="visibility" className="w-full bg-pastel-input border-pastel-input-border focus:border-pastel-input-focus-border">
                    ...
                    <SelectContent className="bg-pastel-popover text-pastel-popover-foreground border-pastel-border">
                      {VISIBILITY_OPTIONS.map(option => (
                        <SelectItem key={option.id} value={option.id} className="hover:bg-pastel-muted focus:bg-pastel-muted">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-pastel-border/60">
              ...
              <Button type="submit" variant="default" disabled={isSubmitting || isUploadingImage} className="min-w-[120px]">
                {isSubmitting || isUploadingImage ? <Loader2 className="w-4 h-4 animate-spin"/> : (recipe ? "Sauvegarder" : "Créer Recette")}
              </Button>
            </div>
          </form>

          {showTagManager && (
            <TagManager onClose={() => setShowTagManager(false)} existingTags={existingTags} setExistingTags={setExistingTags} />
          )}
        </motion.div>
      </motion.div>
    </AnimatePresence>
