...
import { UserCircle, Search, Loader2, Utensils, Users, Compass, Eye } from 'lucide-react';
...
export default function CommunityPage({ session, userProfile }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .or(`username.ilike.%${trimmedSearchTerm}%,user_tag.ilike.%${trimmedSearchTerm}%`)
        .neq('id', session.user.id)
        .limit(10);

      if (error) {
        console.error("Error searching users (raw):", error);
        throw new Error(`Erreur Supabase: ${error.message} (code: ${error.code}, details: ${error.details}, hint: ${error.hint})`);
      }
      setSearchResults(data.map(u => ({...u, username: u.username || u.id.substring(0,8) })));
    } catch (error) {
  ...
  const fetchPublicRecipes = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Erreur", description: "Impossible de charger les recettes publiques: " + error.message, variant: "destructive" });
    } finally {
    ...
    <div className="space-y-8">
      <div className="section-card p-6 sm:p-8">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-pastel-primary mb-3 tracking-tight">Communauté</h1>
          <p className="text-pastel-muted-foreground mb-8 max-w-xl">
            Explorez les créations culinaires, trouvez de l'inspiration et gérez vos connexions.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-8">
            <div className="section-card p-6">
              <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
                ...
                />
                <Button type="submit" variant="primary" disabled={loadingSearch} className="w-full sm:w-auto">
                  {loadingSearch ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Search className="w-5 h-5 mr-2" />}
                  Rechercher
                </Button>
              </form>
            </div>
            ...
            {searchResults.length > 0 && (
              <div className="section-card p-6">
                <h2 className="text-xl font-semibold text-pastel-primary mb-4">Résultats de la recherche :</h2>
                <ul className="space-y-3">
                  {searchResults.map((user) => (
                    <li key={user.id} className="flex items-center justify-between p-3 bg-pastel-card-alt rounded-lg shadow-sm hover:shadow-md transition-shadow">
            ...
            {searchTerm && !loadingSearch && searchResults.length === 0 && (
               <div className="text-center py-8 section-card">
                  <p className="text-lg text-pastel-muted-foreground">Aucun utilisateur trouvé pour "{searchTerm}".</p>
               </div>
            )}

            <div className="section-card p-6">
              <h2 className="text-xl sm:text-2xl font-bold text-pastel-secondary mb-6 text-center">Dernières Recettes Publiques</h2>
              {loadingRecipes ? (
                <LoadingScreen message="Chargement des recettes publiques..." />
              ) : publicRecipes.length > 0 ? (
                <RecipeList 
                  recipes={publicRecipes} 
                  onEdit={() => toast({ title: "Non disponible", description: "Modification non permise ici.", variant: "default"})} 
                  onDelete={() => toast({ title: "Non disponible", description: "Suppression non permise ici.", variant: "default"})} 
                  onSelectRecipe={handleSelectRecipe}
                />
              ) : (
                <div className="text-center py-8">
                  <Utensils className="w-12 h-12 mx-auto mb-3 text-pastel-border"/>
                  <p className="text-lg text-pastel-muted-foreground">Aucune recette publique à afficher pour le moment.</p>
                  <p className="text-pastel-text/70">Soyez le premier à partager une recette !</p>
                </div>
              )}
            </div>
          </div>
