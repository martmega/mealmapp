...
import { PlusCircle, Calendar, ShoppingCart, LogOut, LogIn, UserCircle, Moon, Sun, Star, Utensils, Users } from "lucide-react";
...
export default function MainAppLayout({ 
  ...
  return (
    <>
      <header 
        className="border-b border-pastel-border/60 dark:border-pastel-border/30 shadow-pastel-soft sticky top-0 z-40 bg-white dark:bg-slate-800 bg-opacity-90 dark:bg-opacity-80 backdrop-blur-md"
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl sm:text-3xl font-bold text-pastel-primary dark:text-pastel-primary-hover flex items-center">
              ...
              Mealmapp <span className="text-xs font-normal text-pastel-muted-foreground dark:text-pastel-muted-foreground/80 ml-2 tracking-wider">Alpha</span>
            </h1>
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <Button variant="ghost" size="icon" onClick={toggleDarkMode} className="text-pastel-accent dark:text-pastel-accent-hover hover:bg-pastel-accent/10 dark:hover:bg-pastel-accent/20 rounded-full">
              ...
              {session ? (
                ...
                <Button variant="default" onClick={() => setShowAuth(true)} size="sm">
          ...
          {showMainNavigation && (
            <nav className="flex space-x-1 sm:space-x-2 mt-5 overflow-x-auto pb-1 -mb-px">
              {[
                { id: "recipes", label: "Recettes", icon: PlusCircle, path: "/app/recipes" },
                { id: "menu", label: "Menu", icon: Calendar, path: "/app/menu" },
                { id: "shopping", label: "Courses", icon: ShoppingCart, path: "/app/shopping" },
                ...(session ? [{ id: "community", label: "Communauté", icon: Users, path: "/app/community" }] : []),
                ...(session ? [{ id: "account", label: "Mon Compte", icon: UserCircle, path: "/app/account" }] : [])
              ].map(tab => (
                <Button 
                  key={tab.id}
                  variant="ghost"
                ...
                >
                  <tab.icon className="w-3.5 h-3.5 mr-1.5 hidden sm:inline-block" />{tab.label}
                </Button>
              ))}
            </nav>
          )}
