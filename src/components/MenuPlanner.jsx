...
function MenuPlanner({
  ...
  const handlePlannedServingsChange = (
    ...
    const updatedMenu = internalWeeklyMenu.map((day) =>
      day.map((meal) => [...meal])
    );
  ...
  const handleAddLinkedUser = async () => {
    ...
    if (
      newLinkedUserEmail.trim().toLowerCase() ===
    ...
    try {
      ...
      const peerUsername =
        peerUser.username || peerUser.user_tag || peerUser.id.substring(0, 8);
          ...
          .map((rel) => {
            ...
            return {
              ...
              name:
                ...
                friendProfile.user_tag ||
                friendProfile.id.substring(0, 8),
              ratio: 0,
              isOwner: false,
            };
          })
          ...
          >
            ...
            <DialogContent className="max-w-3xl bg-pastel-card border border-pastel-border rounded-lg dark:bg-pastel-card dark:border-pastel-border">
              <DialogHeader>
                ...
                <DialogDescription>
                  Configurez vos préférences pour la génération automatique du
                  menu.
                </DialogDescription>
              </DialogHeader>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xxl:grid-cols-7">
        {DAYS.map((day, dayIdx) => (
          <DailyMenu
            ...
            dayIndex={dayIdx}
            menuForDay={
              Array.isArray(internalWeeklyMenu) && internalWeeklyMenu[dayIdx]
                ? internalWeeklyMenu[dayIdx]
                : []
            }
