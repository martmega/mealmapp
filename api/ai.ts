...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  switch (action) {
    case 'description':
      return handleDescription(req, res, user);
    case 'image':
      return handleImage(req, res, user);
    case 'cost':
      return handleCost(req, res, user);
    case 'format':
      return handleFormat(req, res, user);
    default:
      return res.status(400).json({ error: 'Invalid action' });
  }
}

async function getSubscriptionTier(userId: string) {
  ...
  return data?.subscription_tier || 'standard';
}
...
async function handleCost(
  ...
  if (user.raw_user_meta_data?.subscription_tier !== 'premium') {
  ...
  try {
    ...
    const ingredientsList = Array.isArray(recipe.ingredients)
      ? recipe.ingredients
          .map(
            (ing: any) => `- ${ing.quantity} ${ing.unit ? ing.unit + ' ' : ''}${ing.name}`.trim(),
          )
          .join('\n')
      : '';
